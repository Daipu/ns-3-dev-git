...
NS_LOG_COMPONENT_DEFINE ("CobaltExample");
...
BuildAppsTest ()
{
  ...
  */
  ...
  clientHelper1.SetAttribute ("PacketSize", UintegerValue (1500));
  ...
  clientHelper2.SetAttribute ("PacketSize", UintegerValue (1500));
...
{
  LogComponentEnable ("CobaltQueueDisc", LOG_LEVEL_INFO);

  std::string cobaltLinkDataRate = "1.5Mbps";
  std::string cobaltLinkDelay = "20ms";
  ...
  // 42 = headers size
  Config::SetDefault ("ns3::TcpSocket::SegmentSize", UintegerValue (1500 - 42));
  ...
  p2p.SetQueue ("ns3::DropTailQueue");
  p2p.SetDeviceAttribute ("DataRate", StringValue (cobaltLinkDataRate));
  p2p.SetChannelAttribute ("Delay", StringValue (cobaltLinkDelay));
  devn2n3 = p2p.Install (n2n3);
  // only backbone link has Cobalt queue disc
  queueDiscs = tchCobalt.Install (devn2n3);
  ...
  if (writeForPlot)
    {
      filePlotQueueDisc << pathOut << "/" << "cobalt-queue-disc.plotme";
      filePlotQueueDiscAvg << pathOut << "/" << "cobalt-queue-disc_avg.plotme";
